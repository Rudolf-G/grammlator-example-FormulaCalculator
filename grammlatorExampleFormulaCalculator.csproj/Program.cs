...
namespace GrammlatorExampleFormulaCalculator {
   /* This program demonstrates how a LALR(1)-grammar can be used together
    * with C# methods as a control structure which is translated to C# code by grammlator.
    ...
    * As can be seen grammlator inserts the produced verbose code in a special
    * region of the source files ReadAndAnalyze... (two versions) and MyLexer.
    * Both parser versions and the lexer havw been programmed using grammlator.
    * 
    ...
    * static priorities of grammar rules (one version of the lexer) and
    * dynamic priorities (the other version of the lexer).
    * These priorities solve these ambiguities according to operator precedence and associativity.
    * 
   ...
   internal static class Program {
      ...
      private static void InputOutputLoop(Dictionary<string, double> DefinedNames)
      {
         ...
         while (true)
         {
            ...
            // var example = new ReadAndAnalyzeWithStaticPriorities(Line);
            example.ReadAndAnalyzeExpression(DefinedNames);

            // ReadAndAnalyzeExpression will call the error handler if it can not recognize a legal expression,
            // for example if you enter '#' (interpreted by myCharInput as "OtherCharacter"-Symbol)
